@extends('layouts.app')
@section('title','View Inland Training')
@section('content')

    <div class="app-title">
        <div>
            <h1><i class="fa fa-users" aria-hidden="true"></i>View Inland Training</h1>
        </div>
        ...
        <div class="col-md-12">
            <div class="card mb-3">
                <div class="card-header bg-info text-white">
                    <span class="card-title">Inland Training Info</span>
                    <a class="btn btn-danger btn-sm icon-btn float-right" href="{{route('local-training.index')}}"><i class="fa fa-list"></i>See List</a>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered text-uppercase" id="" style="width:100%;">
                            ...
                            <tbody>
                            <tr>
                                <td>{{$localTraining->course_title ??''}}</td>
                                <td>{{$localTraining->course_code ??''}}</td>
                                <td>{{$localTraining->created_at ? \Carbon\Carbon::parse($localTraining->created_at)->format('d-m-Y h:i:s A') : ''}}</td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#sampleTable2').DataTable({
                serverSide: true,
                processing: true,
                ajax: '{{ route('local-training.show',$localTraining->id) }}',
                columns:[
