@extends('layouts.app')
@section('title','Assign Employee for Award')
@section('content')

    <div class="app-title">
        <div>
            <h1><i class="fa fa-users" aria-hidden="true"></i> {{$award->award_name}}</h1>
        </div>
    ...
    <div class="row" id="vue_app">
        <div class="col-md-12">
            <form method="POST" action="{{route('add-employee-to-award-store')}}" enctype="multipart/form-data">
                @csrf
            <div class="card mb-3">
        ...
        $(document).ready(function () {
            var vue = new Vue({
                ...
                methods: {
                    ...
                    selectItem(id){
                        if(this.awarded_employees_id.includes(id)){
                            toastr.error('Already Awarded.', {
                                closeButton: true,
                                progressBar: true,
                            });
                    ...
                    formSubmit(){
                        ...
                        axios.post('/award/store-employee', formData, config)
                            .then((response) => {
                                ...
                                if (response.data === 'done') {
                                    ...
                                    window.location = "{{route('award.index')}}"
                                }
                            })
                            .catch((error) => {
                ...
                mounted: function (){
                    ...
                    axios.get('/award/get-employee-id',{
                        params: {
                            award_id: '{{$award->id}}'
                        }
                    }).then((response)=>{
                        this.awarded_employees_id = response.data
                    })
                }
            });
