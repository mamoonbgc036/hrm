@extends('layouts.app')
@section('title','Pending Abroad Trainings')
@section('content')

    <div class="app-title">
        <div>
            <h1><i class="fa fa-users" aria-hidden="true"></i> Pending Employee Abroad Trainings</h1>
        </div>
        ...
        <div class="col-md-12">
            <div class="tile">
                <div class="float-left">
                    <h4><i class="fa fa-list" aria-hidden="true"></i> Pending Abroad Training </h4>
                </div>
                <div>
                    @can('Abroad training create')
                        <a href="{{route('foreign-training.index')}}" class="btn btn-info btn-sm float-right mb-2"
                           data-toggle="tooltip" title="Show All"><i class="fa fa-plus-square-o" aria-hidden="true"></i>All Abroad Training</a>
                    @endcan
                    @can('Abroad training create')
                        <a href="{{route('foreign-training.create')}}" class="btn btn-primary btn-sm float-right mb-2"
                           data-toggle="tooltip" title="Add New"><i class="fa fa-plus" aria-hidden="true"></i>Add Abroad Training</a>
                    @endcan
                </div>
                <div class="tile-body">
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered text-uppercase" id="sampleTable" style="width:100%;">
                            ...
                            <tbody>
                                @foreach($pendingEmployeeToForeignTrainings as $row)
                                    <tr>
                                        <td>{{$loop->index+1}}</td>
                                        <td><a href="{{route('foreign-training.show',$row->foreign_training_id)}}" target="_blank">{{$row->course_title}}</a></td>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                @can('Abroad training approve')
                                                    <a href="{{route('approve-deny-employee-foreign-training',[$row->id, 'approve']) }}" class="btn btn-success btn-sm"
                                                       data-toggle="tooltip" title="Approve"><i class="fa fa-check">Approve</i></a>
                                                @endcan
                                                @can('Abroad training deny')
                                                    <a href="{{route('approve-deny-employee-foreign-training',[$row->id, 'deny']) }}" class="btn btn-danger btn-sm"
