<form action="{{ route('employee-disease') }}" method="post">
    ...
    @if (!$employee->getDiseaseHist->isEmpty())
                            ...
                            <div class="row">
                                <div class="col-4 col-sm-4 col-md-4 col-xl-4">
                                    <div class="form-group">
                                        ...
                                        <select name="disease[]" class="form-control" id="" required>
                                            @if($diseases && $diseases->count() > 0)
                                                <option value="">Select Disease</option>
                                                @foreach ($diseases as $item)
                                                    <option value="{{ $item->id }}" {{ $disease->disease_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                                @endforeach
                                            @else
                                ...
                                <div class="col-4 col-sm-4 col-md-4 col-xl-4">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" id="disease_name" type="text" name="disease_name[]" value="{{ $disease->getDiseaseName->name??null }}">
                                    </div>
                                </div>
                                <div class="col-4 col-sm-4 col-md-4 col-xl-4">
                                    <div class="form-group">
                                        <label class="col-form-label col-form-label-sm" for="disease_description">Description</label>
                                        <textarea class="form-control" id="disease_description" type="text" rows="3" cols="10" name="disease_description[]" >{{ $disease->disease_description??null }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
