<form id="form" action="{{route('export_excel_dynamic')}}">
    @csrf
    <div class="row text-uppercase">
        ...
        <div class="col-md-3 col-sm-4">
            <div class="form-group">
                ...
                <select class="form-control text-uppercase" name="police_station_id" id="police_station_id">
                    <option value="">SELECT OFFICE/STATION</option>
                    @foreach($stations as $value)
                        <option value="{{$value->id}}" {{request('police_station_id')==$value->id?'selected':''}}>{{ App\Classes\StringConversion::stringToUpper($value->name) }}</option>
                    @endforeach
                </select>
            </div>
        </div>
