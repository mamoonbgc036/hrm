...
@section('content')
    ...
    <div class="app-title">
        <div>
            <h1><i class="fa fa-users" aria-hidden="true"></i>Promoted Job History</h1>
        </div>
            ...
            <div class="tile">
                ...
                <div>
                    @can('Job history deleted button')
                        <a href="{{route('posting-record.deleted')}}" class="btn btn-danger btn-sm float-right mb-2"
                           data-toggle="tooltip" title="Deleted"><i class="fa fa-trash" aria-hidden="true"></i> Deleted Job History</a>
                    @endcan
                    @can('Job history create')
                        <a href="{{route('posting-record.create')}}"
                           class="btn btn-primary btn-sm float-right mb-2 mr-1"
                           data-toggle="tooltip" title="Add New"><i class="fa fa-plus-square-o" aria-hidden="true"></i>Add New Job History</a>
                    @endcan
                    @can('Job history transfers button')
                        <a href="{{route('posting-record.transfers')}}" class="btn btn-warning btn-sm float-right mb-2 mr-1"
                    ...
                    @can('Job history list')
                        <a href="{{route('posting-record.index')}}" class="btn btn-info btn-sm float-right mb-2 mr-1"
                           data-toggle="tooltip" title="Show All"><i class="fa fa-plus-square-o" aria-hidden="true"></i> All Job History</a>
                    @endcan
                </div>
                ...
                <div class="tile-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered" id="sampleTable">
                            ...
                            @forelse($promotion_records as $index=>$row)
                                <tr>
                                    <td>{{$index+1}}</td>
                                    <td>{{ App\Classes\StringConversion::stringUpperArray($row->employee->name ?? '') }}</td>
                                    <td>{{ App\Classes\StringConversion::stringUpperArray($row->employee->pin_no ?? '') }}</td>
                                    ...
                                    <td>{{ App\Classes\StringConversion::stringUpperArray($row->grade->grade ?? '') }}</td>
                                    <td>{{ $row->from_date ?? ''}}</td>
                                    ...
                                    <td class="text-right">
                                        <div class="btn-group">
                                            @can('Job history edit')
                                                <a class="btn btn-sm btn-primary" href="{{ route('posting-record.edit',$row) }}"
